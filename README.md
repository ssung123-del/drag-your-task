# 🎯 드래그 유얼 테스크 (Drag Your Task) v1.4
...
## 🆕 최신 업데이트 (v1.4 Highlights)

### 1. 📐 데스크탑 레이아웃 최적화
- **기본 블록 상단 인라인 배치**: 기존 좌측 사이드바에 있던 `심방`/`업무` 블록을 헤더 제목 옆으로 이동하여 타임테이블이 **전체 너비**를 활용할 수 있도록 개선.
- **토요일 칼럼 짤림 해결**: 사이드바 공간을 제거하여 주간 뷰에서 모든 요일이 잘림 없이 표시.
- **컴팩트한 블록 디자인**: 블록 크기를 축소하여 헤더와 시각적으로 조화롭게 통합.
