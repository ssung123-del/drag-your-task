# 🎯 드래그 유얼 테스크 (Drag Your Task) v1.6

> **"사역 기록, 이제는 입력하지 말고 드래그하세요."**  
> 드래그 유얼 테스크는 교역자분들의 효율적인 사역 기록과 주간 보고서 작성을 돕는 스마트 비서 앱입니다.

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=white)

---

## 🆕 최신 업데이트 (v1.6 Highlights)

### 🚀 드래그 성능 대폭 최적화 (v1.6)
- **`handleEdit` 콜백 참조 안정화**: `entriesRef` 패턴을 적용하여 entries 변경 시에도 콜백 참조가 유지되도록 개선. 주간 뷰 72개 슬롯의 불필요한 리렌더링을 완전 제거.
- **`DraggableBlock`에 `React.memo` 적용**: 드래그 중 블록 컴포넌트의 불필요한 리렌더링 방지.
- **`isSameDay` → 문자열 비교로 전환**: 매 슬롯마다 `new Date()` 객체를 생성하던 것을 `todayStr` 캐싱으로 교체하여 연산량 대폭 감소.
- **`entriesBySlot`에서 `BoardEntryItem` 변환 캐싱**: 슬롯 접근 시마다 수행되던 타입 변환을 1회로 축소.

### 🔀 드래그 이동/복사 선택 (v1.5)
- 기존 엔트리를 다른 시간대로 드래그할 때 **이동** 또는 **복사** 중 선택할 수 있는 모달이 표시됩니다.
- **이동**: 원래 위치에서 제거하고 새 위치로 옮깁니다.
- **복사**: 원래 위치는 유지하고 새 위치에 동일한 일정을 복제합니다.

### ⌨️ ESC 키 취소 지원 (v1.5)
- 모든 모달(내용 입력, 수정, 분류 선택)에서 `ESC` 키를 눌러 빠르게 닫을 수 있습니다.

### ⚡ 기존 성능 최적화 (v1.5)
- `React.memo`, `useCallback`을 활용하여 드래그 중 불필요한 리렌더링을 방지.
- Zustand store selector를 개별 분리하여 상태 변경 시 관련 컴포넌트만 업데이트.
- `DroppableTimeSlot`에 커스텀 비교 함수 적용으로 주간 뷰 렌더링 성능 대폭 향상.

### 🎨 라이트 모드 전용화
- 다크 모드 관련 코드를 완전히 제거하여 코드 일관성 및 유지보수성 향상.

### 5. 📐 데스크탑 레이아웃 최적화 (v1.4)
- **기본 블록 상단 인라인 배치**: 헤더 제목 옆으로 이동하여 타임테이블이 **전체 너비**를 활용.
- **토요일 칼럼 짤림 해결** 및 **컴팩트한 블록 디자인**.

### 6. 📝 HWPX 내보내기 기능 강화
- **정교한 템플릿 매핑**: 기존 한글 서식(HWPX)의 셀 좌표에 데이터를 정확하게 입력합니다.
- **다중 입력 지원**: 같은 시간대에 여러 사역 일정을 입력해도 누락 없이 모두 기록됩니다.
- **안정성 확보**: 불안정한 '클립보드 복사' 기능을 제거하고, 파일 직접 다운로드 방식을 채택하여 데이터 손실 위험을 없앴습니다.

### 4. 🛠️ 사용성 개선
- **수정 기능 추가**: 등록된 사역 블록을 다시 클릭하여 내용을 간편하게 수정할 수 있습니다.
- **입력 편의성**: 심방/업무 등 카테고리별 입력 필드를 최적화했습니다.

---

## ✨ 핵심 기능 (Core Features)

### 1. 🖱️ 드래그 앤 드롭 사역 기록
- 복잡한 입력창 대신 **심방**, **업무** 블록을 원하는 시간대에 드래그하여 직관적으로 기록합니다.
- dnd-kit 기반의 부드러운 애니메이션과 터치 대응을 지원합니다.

### 2. ☁️ Google 클라우드 자동 동기화
- Firebase 연동을 통해 **Google 로그인** 시 모든 데이터가 서버에 안전하게 저장됩니다.
- 아이폰에서 기록하고 PC에서 즉시 확인하는 실시간 동기화 환경을 제공합니다.

### 3. 📱 멀티 플랫폼 반응형 디자인
- **데스크톱/태블릿**: 넓은 화면을 활용한 주간 타임테이블 뷰. 헤더에 블록을 인라인 배치하여 공간 효율 극대화.
- **모바일**: 한 손 조작에 최적화된 하단 탭 바와 카드형 레이아웃.
- Apple 디자인 가이드를 준수하는 프리미엄 UI/UX.

### 4. 📊 스마트 주간 보고서 (Excel & HWPX)
- 주간 사역 데이터를 분석하여 정해진 양식의 **Excel 및 HWPX 파일**로 즉시 내보내기 가능.
- **HWPX 템플릿 지원**: 기존 한글(HWP) 서식의 스타일(폰트, 문단 모양)을 그대로 유지하며 데이터를 자동 입력합니다.
- 수동 정리가 필요 없는 자동화된 보고 프로세스.

---

## 🛠 기술 스택 (Tech Stack)
- **Frontend**: React (Functional Components, Hooks)
- **State Management**: Zustand (with Persist Middleware)
- **Styling**: Tailwind CSS
- **Backend**: Firebase Store (Firestore), Firebase Auth (Google)
- **Utilities**: Date-fns, Lucide-React, ExcelJS, JSZip, clsx

---

## 🚀 시작하기 (How to Start)

### 로컬 실행
```bash
npm install
npm run dev
```

### 환경 설정
- `.env` 파일에 Firebase 설정을 입력하면 클라우드 기능을 사용할 수 있습니다.
- 예시:
```bash
cp .env.example .env
```
- 필요한 키:
  - `VITE_FIREBASE_API_KEY`
  - `VITE_FIREBASE_AUTH_DOMAIN`
  - `VITE_FIREBASE_PROJECT_ID`
  - `VITE_FIREBASE_STORAGE_BUCKET`
  - `VITE_FIREBASE_MESSAGING_SENDER_ID`
  - `VITE_FIREBASE_APP_ID`

---

## 📋 사역자 가이드
1. **Google 로그인**: 하단 버튼을 통해 로그인하여 동기화를 시작합니다.
2. **사역 블록 드래그**: 헤더의 블록(심방/업무)을 타임라인으로 끌어옵니다.
3. **상세 내용 입력**: 나타나는 모달에서 세부 유형과 내용을 입력합니다.
4. **Excel 다운로드**: [내보내기] 탭에서 주간 보고서를 생성합니다.

---

**Designed for Ministry Efficiency**  
Copyright © 2026 Drag Your Task Team. All rights reserved.
